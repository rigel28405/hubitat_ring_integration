...
def mainPage() {
  ...
  dynamicPage(name: "mainPage", title: "Manage Your Ring Devices", uninstall: true, install: true) {
    ...
    section("Advanced options", hidden: true, hideable: true) {
      ...
      paragraph('<b style="color: red;">Do not toggle this button without understanding the following.</b> Resetting this token will require you to manually update <b>ALL</b> of the URLs in any existing dashboard tile any IFTTT appletd. There is no need to reset the token unless it was compromised.')
...
def ifttt() {
  ...
  dynamicPage(name: "ifttt", title: '<b style="font-size: 25px;">Using IFTTT To Receive Motion and Ring Events</b>') {
    section('<b style="font-size: 22px;">About IFTTT</b>') {
      ...
      paragraph("For the above to function correctly a few things must be setup. This app (the Hubitat \"Unofficial Ring Connect\") must be configured for OAuth. This is so to allow authenticated, wanted, inbound web service calls that can be routed to your hub.")
      if (!isOAuthEnabled()) {
        paragraph('<b style="color: red;">OATH is not currently enabled under the "OAuth" button in the app code editor for this app. This is required to use IFTTT to receive motion and ring events.</b>')
      }
      paragraph("An IFTTT applet must be configured for each event type for each device.")
    }
    section('<b style="font-size: 22px;">Prerequisites</b>') {
      paragraph("""- OAuth enabled on this app. You can do this from the "Apps Code" section of the Hubitat UI (Not necessary if installed with HPM)
- An IFTTT account
...
- The last GUID is the access token created by this app using OAuth that IFTTT will use to authenticate to Hubitat. <b>DO NOT</b> disclose this to anybody. It is like a password.""")
    ...
    section('<b style="font-size: 22px;">Body Payloads for Ring Events') {
      paragraph(
        dingables.findAll{ RINGABLES.contains(it.getDataValue("kind")) }.collect {
...
<li>OAuth enabled on this app. You can do this from the "Apps Code" section of the Hubitat UI (Not necessary if installed with HPM)</li>
<li>A device that supports snapshots and has snapshots enabled on the phone app's dashboard</li>
</ul>""")
    }

    if (!isOAuthEnabled()) {
      section('<h2>OAuth Error</h2>') {
        paragraph('<b style="color: red;">OAUTH is not currently enabled for this app. You must enable it to continue</b>')
      }
    }
...
boolean isClientDeviceNotFoundHttpError(errorData, final Integer status, final String uri, final String functionName) {
  if (status == 404) {
    ...
    try {
      Map errorJson = parseJson(errorData)
...
void schedulePeriodicMaintenance() {
  schedule("0 3 3 ? * MON", periodicMaintenance)
}
...
boolean isOAuthEnabled() {
  try {
    ...
    logDebug ("isOAuthEnabled exception: ${ex}")
    return false
  }
}
