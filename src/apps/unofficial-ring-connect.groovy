    ...
    section('<b style="font-size: 22px;">Body Payloads for Motion Events</b>') {
      paragraph(
        state.dingables.collect {
          "<u>" + getChildDeviceInternal(it).label + ":</u>\n" +
    ...
    section('<b style="font-size: 22px;">Required Device Configurations</b>') {
      ...
      paragraph(
        "<u>To configure snapshots you will need to activate \"Enable polling for thumbnail snapshots on this device\" for the device you want to see. The links below will bring you to the device configuration pages</u>:\n\n" +
          state.dingables.collect {
            def d = getChildDeviceInternal(it)
    ...
    section('<b style="font-size: 22px;">Snapshots</b>') {
      paragraph(
        state.snappables.findAll { it.value == true }.collect {
          ...
          "<u><b>" + getChildDeviceInternal(it.key).label + ":</b></u>\n" +
            "<b>URL</b>: ${url}\n" +
...
def deviceDiscovery(Map params = [:]) {
  ...
  if (params.reset == "true") {
    ...
    getAPIDevice().setAlarmCapable(false)
  }

  discoverDevices()
  Map devices = devicesDiscovered()

  logTrace "devices ${devices}"

  Map options = devices ?: [:]
  Integer numFound = options.size() ?: 0
...
Map devicesDiscovered() {
  List vdevices = getDevices()

  logTrace "vdevices ${vdevices}"

  Map map = [:]
  vdevices.each {
    final String value = it.name.toString()
    final String key = it.id.toString()
...
private discoverDevices() {
  ...
  boolean alarmCapable = (state.devices.find { ALARM_CAPABLES.contains(it.kind) }?.size() ?: 0) > 0
  getAPIDevice().setAlarmCapable(alarmCapable)
}
...
mappings {
  path("/notify") {
    action: [POST: "processNotification"]
  }
  path("/ifttt") {
...
void processIFTTT() {
  ...
  logTrace "data received: kind: ${kind}, id: ${json.id}, device: ${d}, ${params}, request: ${request}, data: ${request.body}"
...
def addDevices() {
  ...
  selectedDevices.each { id ->
    ...
    def d
    if (selectedDevice) {
      d = getChildDeviceInternal(selectedDevice.id)
    }
    ...
    if (!d) {
      ...
      try {
        if (isHub(selectedDevice.kind)) {
          createableHubs.add(selectedDevice.kind)
          ...
          final Map deviceType = DEVICE_TYPES[selectedDevice.kind]
          log.warn "Creating a ${deviceType.name} with dni: ${getFormattedDNI(selectedDevice.id)}"
          def newDevice = addChildDevice("ring-hubitat-codahq", deviceType.driver, getFormattedDNI(selectedDevice.id), selectedDevice?.hub, [
            "label": selectedDevice?.name ?: deviceType.name,
            "data": [
              "device_id": selectedDevice.id,
              "kind": selectedDevice.kind,
              "kind_name": deviceType.name
            ]
          ])
          newDevice.refresh()
          sectionText += "Successfully added ${deviceType.name} with DNI ${getFormattedDNI(selectedDevice.id)} \r\n"
        }
      }
      catch (e) {
        final Map deviceType = DEVICE_TYPES[selectedDevice.kind]
        if (e.toString().replace(deviceType.driver, "") ==
          ...
          final String msg = '<b style="color: red;">The "' + deviceType.driver + '" driver for device "' +
            deviceType.name + '" was not found and needs to be installed.</b>\r\n'
          log.error msg
          sectionText += msg
        }
        else {
  ...
  apiDevice.setCreateableHubs(createableHubs)

  if (hubAdded) {
    // Setup websocket connection. Will trigger a refresh and add the hub zids to the API device's state.
    // If hub devices have not been created, this will create them automatically
...
def getDeviceIds() {
  ...
  return json.inject([]) {
    acc, node ->
      ...
      logTrace "node: ${node}"
      if (DEVICE_TYPES[node?.kind] && settings.selectedLocations.contains(node?.location_id)) {
...
void configureSnapshotPolling() {
  ...
  if (snapshotPolling) {
    logInfo "Snapshot polling started with an interval of ${snapshotInvertals[snapshotInterval.toInteger()].toLowerCase()}."
    setupDingables()
    ...
    switch (interval) {
      case 30:
        final String secString = currSec > alt ? "${altSec},${currSec}" : "${currSec},${altSec}"
...
void prepSnapshots() {
  ...
  simpleRequest("snapshot-timestamps", [snappables: snappables.collect { getRingDeviceId(it.key).toInteger() }])
...
void getSnapshots() {
  ...
  state.snappables.findAll { it.value }.each {
...
def serveSnapshot() {
  ...
  // Get the device
  def d = getChildDeviceInternal(ringDeviceId)
  if (d == null) {
  ...
  imageResponse(state.snapshots[ringDeviceId]?.toArray() as byte[])
}
...
"""
  ...
  */
  ...
  if (!img || img.length == 0) {
    ...
    log.info "Default to missing image"
    strImg = MISSING_IMG
  }
  else {
...
@Field final static Map<Integer, String> snapshotInvertals = [
  30: "30 Seconds",
  60: "1 Minute",
  90: "1.5 Minutes",
  120: "2 Minutes",
...
private Map getRequests(final String type, parts) {
  ...
  if (type == "auth") {
    ...
    return [
      ...
      query: [locationID: selectedLocations.toString()]
    ]
  }
    ...
    return [
      ...
      params: [
        ...
        requestContentType: JSON,
        body: [
          mode: parts.mode.toString(), readOnly: true
        ]
      ],
      headers: [
  ...
  else if (type == "refresh-security-device") {
    return [
      method: GET,
      params: [
        uri: "https://api.ring.com",
  ...
  else if (type == "pref-security-device") {
    return [
      method: GET,
      params: [
        uri: "https://app.ring.com",
        path: "/api/v1/rs/preferences/devices/${parts.dni}",
        query: [deviceIdType: 'zid', deviceType: parts.type, userId: parts.user_id, locationId: selectedLocations],
        contentType: JSON
      ]
    ]
  }
  ...
  else if (type == "snapshot-timestamps") {
    return [
      method: POST,
      type: "bearer",
      params: [
  ...
  else if (type == "snapshot-image-tmp") {
    return [
      method: GET,
      synchronous: true,
      type: "bearer",
...
def simpleRequest(final String type, Map data = [:]) {
  ...
  Map request = getRequests(type, data)
  logTrace "request: ${request}"

  Map params = formatParams(request)
...
Map formatParams(final Map request) {
  logDebug "formatParams(request)"
  ...
  logTrace "params: ${JsonOutput.toJson(params)}"
  return params
}

void doAction(final String type, final String method, Map params, Map data) {
...
def doSynchronousAction(final String type, final String method, Map params) {
...
def responseHandler(response, Map params) {
  ...
  if (response.status == 401) {
    ...
    if (params.method == "auth") {
      ...
      boolean result = state.access_token && state.access_token != "EMPTY" && state.refresh_token
      if (result) {
      ...
      logTrace "body: ${JsonOutput.toJson(body)}"
      body.each { deviceInfo ->
        logTrace "deviceInfo: ${deviceInfo}"
        logDebug "refreshing device ${getFormattedDNI(deviceInfo.id)}"
        getChildDeviceInternal(deviceInfo.id)?.childParse(params.method, [response: response.getStatus(), msg: deviceInfo])
      }
    }
    ...
    //  getChildDeviceInternal(data.dni).childParse(type, [response: resp.getStatus(), msg: body])
    //}
      ...
      state.dingables.each { deviceId ->
        ...
        getChildDeviceInternal(deviceId)?.childParse(params.method, [response: response.getStatus(), msg: dingInfo])
      }
    }
    else {
...
def getChildDeviceInternal(id) {
  if (id instanceof String) {
    ...
    } else if (id.startsWith(RING_API_DNI)) {
      return getAPIDevice()
    }
  }

  final String formattedDNI = getFormattedDNI(id.toString())

  def d = getChildDevice(formattedDNI)

  if (!d) {
    def oldDNI = getChildDevice('RING-' + id.toString())

    if (oldDNI) {
      log.warn "Migrated existing device ${oldDNI.deviceNetworkId} to new DNI ${formattedDNI}..."
...
def getAPIDevice(Map location = null) {
  ...
  if (!d) {
    ...
    //create otherwise
      ...
      d.initialize()
      d.refresh()
      logInfo "${deviceType.name} with ID ${formattedDNI} created..."
    }
  }
  return d
}
...
String getRingDeviceId(final String dni) {
  ...
  List split = dni.split("\\|\\|")

  if (split?.size() != 2) {
    split = dni.split('-')
  }
  return split?.getAt(1)
}

void generateAppDeviceId() {
